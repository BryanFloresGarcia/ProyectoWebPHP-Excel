...
if (isset($_FILES["fileCustom"]["name"])) {
    ...
    if ($_REQUEST['selecttabla'] == "Seleccione_una_tabla") {
        $_SESSION['nombreDeTabla'] = preg_replace('/[^a-zA-Z0-9_]/', '', str_replace('_', '', $_REQUEST['nombreTabla']));
    }else {
        $_SESSION['nombreDeTabla'] = preg_replace('/[^a-zA-Z0-9_]/', '', str_replace('_', '', $_REQUEST['selecttabla']));
    }
    
    if ($_SESSION['nombreDeTabla'] == "" && $_REQUEST['eliminar'] == "eliminar") {
    ...
    }else if ($_SESSION['nombreDeTabla'] == "") {
        $_SESSION['data'] = $_FILES["fileCustom"]["tmp_name"] . "";
        $_SESSION['rpta'] = 10;
        header('Location: ../index.php?respuesta=1');
    } else {
